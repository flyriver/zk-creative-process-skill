...
description: >
  ...
  "$zk-creative-process", "single mode", "mix mode", "creative materials",
  "reference deconstruction", or when a video file is provided alongside a
  request for hook/structure/direction analysis.
---
...
Verify with `bash ~/.cursor/skills/zk-creative-process/scripts/check-environment.sh`.
...
```bash
bash ~/.cursor/skills/zk-creative-process/scripts/process-reference-video-phase1.sh \
...
```bash
bash ~/.cursor/skills/zk-creative-process/scripts/process-reference-videos-mix.sh \
